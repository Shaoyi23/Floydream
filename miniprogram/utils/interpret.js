const SYMBOL_LIBRARY = [
  {
    symbol: '飞行或漂浮',
    keywords: ['飞', '飞行', '漂浮', '升空'],
    meaning: '通常和想突破现实边界、追求自由感有关。',
  },
  {
    symbol: '坠落',
    keywords: ['坠落', '掉下去', '下坠'],
    meaning: '常见于压力偏高或对失控感比较敏感的时候。',
  },
  {
    symbol: '追逐',
    keywords: ['追', '逃跑', '躲避', '被追'],
    meaning: '像是在提醒你，现实里也许有一个问题正在被回避。',
  },
  {
    symbol: '水',
    keywords: ['海', '河', '雨', '水', '洪水'],
    meaning: '通常和情绪流动、潜意识感受以及安全感有关。',
  },
  {
    symbol: '房间与建筑',
    keywords: ['房间', '房子', '学校', '电梯', '楼梯'],
    meaning: '往往映射个人边界、成长阶段或熟悉环境里的变化。',
  },
  {
    symbol: '人物关系',
    keywords: ['妈妈', '爸爸', '朋友', '前任', '同学', '陌生人'],
    meaning: '更像是在处理关系、期待或未说出口的情绪。',
  },
]
...
function matchSymbols(text) {
  return SYMBOL_LIBRARY.filter((item) => item.keywords.some((keyword) => text.includes(keyword)))
}

function buildLocalInterpretation(dream) {
  const text = `${dream.title || ''} ${dream.content || ''} ${(dream.tags || []).join(' ')}`
  const symbols = matchSymbols(text).slice(0, 3)
  ...
  return {
    ...
    symbols: symbols.length
      ...
      : [
          {
            name: '核心场景',
            meaning: '先关注最反复出现的地点、动作和人物，它们通常比情节顺序更重要。',
          },
        ],
    insight: moodInsight,
...
function normalizeCloudResult(result) {
  ...
  if (result.summary || result.symbols || result.insight) {
    return result
  }
...
function interpretDream(dream) {
  ...
  return wx.cloud.callFunction({
    ...
    const analysis = normalizeCloudResult(res.result)
    ...
    return analysis
  }).catch(() => buildLocalInterpretation(dream))
}
